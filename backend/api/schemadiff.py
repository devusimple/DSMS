        ...
        for name in order:
            ...
            entries.append(
                DiffEntry(
                    ...
                    sql=[
                        create_table_sql(
                            TableSpec(name, list(snap.columns)),
                            dialect,
                            snap.foreign_keys,
                        )
                    ]
                    + [
                        create_index_sql(
                            name, idx.name, idx.columns, idx.unique, dialect
                        )
...
def _diff_table(
    ...
    changed = [
        ...
        if c.name in tgt_cols
        and _column_identity(c) != _column_identity(tgt_cols[c.name])
    ]
...
def _fk_changes(
    src: TableSnapshot, tgt: TableSnapshot
) -> tuple[list[ForeignKeySpec], list[ForeignKeySpec]]:
...
def _column_change_detail(old: ColumnSpec, new: ColumnSpec) -> str:
    ...
    if (old.default or None) != (new.default or None):
        bits.append(
            f"default {new.default!r}" if new.default is not None else "drop default"
        )
...
def _sqlite_rebuild_entry(
    ...
    final_fks = [
        fk for fk in tgt.foreign_keys if _fk_identity(fk) not in drop_identities
    ]
    ...
    for name, idx in src_idx.items():
        ...
        entries.append(
            DiffEntry(
                ...
                sql=[
                    create_index_sql(tgt.name, name, idx.columns, idx.unique, dialect)
                ],
            )
        )
    return entries
...
def migration_script(entries: list[DiffEntry], dialect: str) -> str:
    ...
    lines = [
        f"-- DSMS migration ({dialect})",
        "-- Generated from a schema diff; review before applying.",
    ]
    if not entries:
    ...
    for entry in entries:
        lines.append("")
        lines.append(
            f"-- {entry.detail}" if entry.detail else f"-- {entry.kind} {entry.table}"
        )
