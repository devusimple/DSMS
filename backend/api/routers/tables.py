...
from api.schemas import ColumnDef, GeneratedSql, TableCreate, TableInfo
from api.sqlgen import ColumnSpec, TableSpec, create_table_sql, drop_table_sql
...
def _with_auto_columns(
    cols: list[ColumnSpec], exclude: list[str] | None = None
) -> list[ColumnSpec]:
    ...
    if (
        "_id" not in names
        and "_id" not in excluded
        and not any(c.primary_key for c in cols)
    ):
...
@router.post("/connections/{conn_id}/tables", response_model=TableInfo, status_code=201)
def create_table(
    conn_id: str,
    payload: TableCreate,
    ...
    if ops.table_exists(record, payload.name):
        raise HTTPException(
            status_code=409, detail=f"Table {payload.name!r} already exists."
        )
    table = TableSpec(
        payload.name,
        _with_auto_columns(
            [_column_spec(c) for c in payload.columns], payload.exclude_auto
        ),
    )
