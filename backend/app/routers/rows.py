...
def list_rows(
    ...
    if where:
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid where JSON: {exc}") from exc
        if not isinstance(raw, list) or not all(
            isinstance(item, list) and len(item) == 3 and isinstance(item[0], str) and isinstance(item[1], str)
            for item in raw
        ):
            raise HTTPException(status_code=400, detail="where must be a JSON array of [column, operator, value] rows.")
        where_filters = [(item[0], item[1], item[2]) for item in raw]

    paginated = page is not None or page_size is not None or search or where_filters or order_by
    if not paginated:
        columns, rows = ops.fetch_rows(record, table_name)
        return RowsOut(columns=columns, rows=[_json_values(r) for r in rows], total=len(rows))
