...
from app.schemas import ColumnDef, ColumnUpdate
...
def update_column(
    ...
    if current is None:
        raise HTTPException(status_code=404, detail=f"Column {column_name!r} does not exist.")
    ...
    if renamed and any(c.name == target.name for c in columns):
        raise HTTPException(status_code=409, detail=f"Column {target.name!r} already exists.")
    definition_changed = (
    ...
    try:
        if renamed:
            with record.engine.begin() as conn:
                conn.execute(text(rename_column_sql(table_name, column_name, target.name, record.dialect)))
...
def _apply_definition_change(record, table_name: str, target: ColumnSpec) -> None:
    ...
    columns = [target if c.name == target.name else c for c in ops.reflect_columns(record, table_name)]
