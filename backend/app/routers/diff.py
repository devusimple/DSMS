...
from app.schemas import DiffEntryOut, DiffOut, DiffRequest
from app.schemadiff import diff_schemas, migration_script
...
def diff_connection(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Unknown source connection {payload.source_conn_id!r}.") from exc
    ...
    return DiffOut(
        ...
        entries=[
            DiffEntryOut(kind=e.kind, table=e.table, column=e.column, detail=e.detail, sql=e.sql)
            for e in entries
        ],
