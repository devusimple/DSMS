...
from app.db.manager import ConnectionFailed, ConnectionRecord, manager
...
def create_connection(payload: ConnectionCreate) -> ConnectionOut:
    from app.db.manager import ConnectionSpec
...
def delete_connection(conn_id: str) -> Response:
    if not manager.close(conn_id):
        raise HTTPException(status_code=404, detail=f"Unknown connection id {conn_id!r}")
